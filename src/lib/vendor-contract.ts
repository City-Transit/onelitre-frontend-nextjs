...
function paymentsSectionBody(
  ...
  if (hasCustomSplit) {
    return `Customers pay Onelitre 100% of the order value upfront — you are not paid by the customer directly. Your kitchen has a custom-negotiated payout split, agreed separately from the standard/certified tiers described elsewhere: ${effectiveSplit.advancePct}% of the order value as an advance, and ${effectiveSplit.remainderPct}% within 24 hours of confirmed delivery, with Onelitre retaining a ${effectiveSplit.commissionPct}% platform commission. Payout method and schedule details are confirmed separately from this agreement.`;
  }
  return `Customers pay Onelitre 100% of the order value upfront — you are not paid by the customer directly. Onelitre pays your share out of that amount in two parts. Standard kitchens receive 40% of the order value as an advance, and the remaining 40% within 24 hours of confirmed delivery; Onelitre retains a 20% platform commission. Certified kitchens (see §8) receive 60% as an advance and 20% within 24 hours of confirmed delivery, on the same 20% commission. Your kitchen currently receives the ${effectiveSplit.advancePct}% advance / ${effectiveSplit.remainderPct}% remainder / ${effectiveSplit.commissionPct}% commission split. Payout method and schedule details are confirmed separately from this agreement.`;
}
...
export function buildContractSections(
  ...
  return [
    ...
    },
    {
      ...
      body: paymentsSectionBody(effectiveSplit, hasCustomSplit),
    },
    {
    ...
    },
    {
      ...
      body: 'Before your kitchen can be approved and go live, you must provide a NIN, verified against government records. This is a baseline requirement for every kitchen and is separate from Certified status (§8) — it doesn’t affect your advance rate, only whether you can operate on the platform at all. Providing false identity information is grounds for suspension.',
    },
    {
