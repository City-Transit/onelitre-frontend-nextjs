...
import { VENDOR_CONTRACT_SECTIONS } from '@/lib/vendor-contract';
...
const NUMBER_DOCUMENT_TYPES: { type: VendorDocumentType; label: string; placeholder: string }[] = [
  ...
  { type: 'bvn', label: 'BVN', placeholder: '12345678901' },
...
export function VerificationPanel({
  ...
  async function respondToContract(action: 'accept' | 'decline') {
    ...
    try {
      const updated: Vendor = await apiFetch(`/vendor/contract/${action}`, { method: 'POST' });
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <div className="mt-3 max-h-64 overflow-y-auto rounded-[10px] border border-[rgba(18,33,29,0.14)] bg-paper-dim p-4 text-sm">
          {VENDOR_CONTRACT_SECTIONS.map((section) => (
        ...
        {contractAcceptedAt ? (
          <p className="mt-3 text-sm text-green-700">
            Accepted on {new Date(contractAcceptedAt).toLocaleDateString()}.
          </p>
        ) : contractDeclinedAt ? (
          <div className="mt-3">
            <p className="text-sm text-red-700">
              You declined these terms on {new Date(contractDeclinedAt).toLocaleDateString()}.
              You can&apos;t list meals until you accept them — contact Onelitre support if
              you&apos;d like to reconsider, or accept below.
            </p>
            <button
              onClick={() => respondToContract('accept')}
              disabled={contractBusy}
              className="mt-3 rounded-full bg-paprika-dim px-5 py-2.5 text-sm font-semibold text-white hover:bg-paprika disabled:opacity-65"
            >
